...
Although this statistical problem is well known in the literature, it is not always properly reported as reported by Turhan [@TurhanESE12]. 
...
However instance selection that needs further research (a few exceptions for effort estimation include [@KS02],[@CHPB05]. 

For example, the JM1 dataset rom NASA's defect prediction datasets in PROMISE has 8,000 repeated instances. Defect prediction datasets tend to be are highly unbalanced.


Futhermore, feature selection algorithms do not perform well with imbalanced datasets, resulting in a selection of metrics that are not adequate for the learning algorithms See [@WangKWN12][@RRCA07]
