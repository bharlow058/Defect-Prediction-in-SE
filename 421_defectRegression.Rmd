...
The purpose of this Section is to show the basic ways of estimating software defects by means of several regression models. We apply the techniques on the inputOOFile dataset. In what follows we use the same data points for training and testing, for the sake of clarity.
...
auxfile <- read.arff("./datasets/defectPred/OO/ant-1.7.arff")
myvars <- c('bug', 'wmc', 'rfc', 'cbo', 'lcom', 'loc')
...
inputOOFile <- auxfile[myvars] 
names(inputOOFile)[names(inputOOFile) == 'bug'] <- 'count'
```
...
```{r histogram}
hist(inputOOFile$count,breaks="FD")

## histogram with x axis in log10 scale
ggplot(inputOOFile, aes(count)) +
  geom_histogram() +
  scale_x_log10()
```
...
inputOOFilemat <- data.matrix(inputOOFile, rownames.force = NA)

inputOOFilematcor <- cor(inputOOFilemat, use="complete.obs", method = c("pearson", "kendall", "spearman"))
inputOOFilematcor

corrgram(inputOOFilematcor, order=TRUE, lower.panel=panel.shade,
  upper.panel=panel.pie, text.panel=panel.txt,
  main="Correlation among variables")

corrgram(inputOOFilematcor, order=TRUE, lower.panel=panel.ellipse,
  upper.panel=panel.pts, text.panel=panel.txt,
  diag.panel=panel.minmax,
  main="Correlation among variables")


corrplot(inputOOFilematcor, type = "upper", order = "hclust", 
...
defects_normal <- glm(count ~ cbo + lcom + wmc + rfc ,
                      data = inputOOFile, family = gaussian)
# summary of results
...
data = inputOOFile, family = poisson)
# summary of results
...
defects_zip <- zeroinfl(count ~ cbo + lcom + wmc + rfc | loc, data =
inputOOFile, link = "logit", dist = "poisson", trace = FALSE)
# summary of results
summary(defects_zip)
```
...
# the | seperates the count model from the logistic model
defects_zinb <- zeroinfl(count ~ cbo + lcom + wmc + rfc | loc, data =
inputOOFile, link = "logit", dist = "negbin", trace = FALSE, EM = FALSE)
# summary of results
...
f <- function(data, i) {
  require(pscl)
  m <- zeroinfl(count ~ cbo + lcom + wmc + rfc | loc,
    data = data[i, ], dist = "poisson",
    start = list(count = c(-0.6644, 0.0041, -3e-04, -0.0012, 0.0127), zero = c(2.1588, -0.0066)))
  as.vector(t(do.call(rbind, coef(summary(m)))[, 1:2]))
}

set.seed(10)
(res <- boot(inputOOFile, f, R = 1200, parallel = "snow", ncpus = 4))
...
AIC(defects_normal, k = log(nrow(inputOOFile)))
AIC(defects_pois, k = log(nrow(inputOOFile)))
AIC(defects_negbinom, k = log(nrow(inputOOFile)))
AIC(defects_zip, k = log(nrow(inputOOFile)))
AIC(defects_zinb, k = log(nrow(inputOOFile)))
```
...
# histogram 1
hist(inputOOFile$count, prob = TRUE, col = "gray90", breaks=seq(min(inputOOFile$count)-0.5,
max(inputOOFile$count)+.5, 1), xlab = "Skips Category", ylim=c(0,.8))
...
xlab="Observation")
abline(h=(4/nrow(inputOOFile)), col="red", lwd=2)
...
g.cooks.zi<-function(model){
  n <- nrow(inputOOFile)
  cooks <- as.matrix(rep(0,nrow(inputOOFile)))
  for (i in 1:n){
    if(model=="ZIP"){
      defects_zip.red <- zeroinfl(count ~ cbo + lcom + wmc + rfc | loc, data = inputOOFile[-i,],
    ...
    if(model=="NB"){
      defects_zinb.red <- zeroinfl(count ~ cbo + lcom + wmc + rfc | loc, data = inputOOFile[-i,], link = "logit", dist = "negbin", trace = FALSE, EM
                                   = FALSE)
...
abline(h=(4/nrow(inputOOFile)), col="red")
...
### ZIP Model variable selection for the inputOOFile using AIC and BIC

```{r build models, echo=FALSE}
# Select Final Model
# define the ZIP models to compare
cand.models <- list( )
cand.models[[1]] <- zeroinfl(count ~ loc | loc, data = inputOOFile, link = "logit", dist = "poisson", trace = FALSE)
cand.models[[2]] <- zeroinfl(count ~ loc + cbo | loc, data = inputOOFile, link = "logit", dist = "poisson", trace = FALSE)
cand.models[[3]] <- zeroinfl(count ~ loc + cbo + lcom | loc, data = inputOOFile, link = "logit", dist = "poisson", trace = FALSE)
cand.models[[4]] <- zeroinfl(count ~ loc + cbo + lcom + wmc| loc, data = inputOOFile, link = "logit", dist = "poisson", trace = FALSE)
cand.models[[5]] <- zeroinfl(count ~ loc + cbo + lcom + wmc + rfc| loc, data = inputOOFile, link = "logit", dist = "poisson", trace = FALSE)
cand.models[[6]] <- zeroinfl(count ~ cbo + lcom + wmc + rfc| loc, data = inputOOFile, link = "logit", dist = "poisson", trace = FALSE)
cand.models[[7]] <- zeroinfl(count ~ lcom + wmc + rfc| loc, data = inputOOFile, link = "logit", dist = "poisson", trace = FALSE)
cand.models[[8]] <- zeroinfl(count ~ wmc + rfc| loc, data = inputOOFile, link = "logit", dist = "poisson", trace = FALSE)
cand.models[[9]] <- zeroinfl(count ~ wmc | loc, data = inputOOFile, link = "logit", dist = "poisson", trace = FALSE)
cand.models[[10]] <- zeroinfl(count ~ rfc | loc, data = inputOOFile, link = "logit", dist = "poisson", trace = FALSE)
...
results
# # final model
defects_final_zip <- zeroinfl(count ~ cbo + lcom + wmc + rfc| loc, data = inputOOFile, link = "logit", dist = "poisson", trace = FALSE)
defects_final_zip
```
